...
"""
device_cleanup – плагин для поиска и очистки «тяжёлых» файлов (≥ 2 GB)
...
def register(main_window):
    ...
    lbl = QLabel("Search for files > 2 GB (smart: runs in a separate thread).")
    ...
    btn_pull   = QPushButton("Copy selected")
    ...
    def start_scan():
        ...
        if scan_thread and scan_thread.is_alive():
            QMessageBox.warning(tab, "Scanning", "Scanning is already in progress.")
            return
    ...
    def delete_selected():
        ...
        if not rows:
            QMessageBox.information(tab, "Delete", "No files selected.")
            return

        confirm = QMessageBox.question(
            tab,
            "Confirmation",
            f"Are you sure you want to delete {len(rows)} file(s) from the device?",
        ...
        QMessageBox.information(tab, "Done", "Selected files have been deleted.")
    ...
    def pull_selected():
        rows = get_selected_rows()
        if not rows:
            QMessageBox.information(tab, "Copy", "No files selected.")
            return

        # Выбор папки назначения
        dest_dir = QFileDialog.getExistingDirectory(
            tab, "Select destination folder", os.path.expanduser("~")
        )
        if not dest_dir:
            return
        ...
        QMessageBox.information(
            tab,
            "Done",
            f"Selected files have been copied to {dest_dir}.",
        )
    ...
    # --------------------------------------------------------------
    main_window.tabs.addTab(tab, "Device cleanup")
